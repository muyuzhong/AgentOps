...
- modes that map to the **same** `(event, command)` collapse into **one** proposal (e.g. `undeclared_change` + `declared_not_changed` → one Stop / `check-session-log` proposal), with `failure_codes` listing all contributing codes and `evidence` merging their N/M + hot paths; deterministic order (by command, then first contributing code);
...
- `rationale` cites the recurrence ("`undeclared_change` recurred in N/M evals; a Stop hook running `agentops check-session-log` flags a session that ended without declaring its changes").

These commands already exist (`check-session-log` since Phase 3.5, `eval` since Phase 4) — Phase 6 proposes wiring them, it does not invent new runtime behavior.
...
- [ ] Confirm failure, implement the dataclasses + enum additions (pure stdlib, Chinese comments), run tests (PASS).
- [ ] Commit `feat: add improvement-asset core models`.
...
- [ ] Confirm failure, implement the deterministic projection (provisional `INSTRUCTION_LINE_BUDGET`, documented in code), run tests (PASS).
- [ ] Commit `feat: project rule candidates into instruction suggestions`.

## Task 3: Hook proposals projection

**Files:** `agentops/improve/hooks.py`, `tests/test_improve_hooks.py`.

- [ ] Write failing tests: `derive_hook_proposals` emits one proposal per distinct `(event, command)` for failure modes at or above the recurrence threshold; sub-threshold modes emit nothing; `undeclared_change` + `declared_not_changed` collapse into one Stop / `check-session-log` proposal with both codes in `failure_codes` and merged evidence; `cross_module_breadth` / `confirmed_drift` produce a Stop / `eval` proposal; `settings_snippet` is the deterministic, copy-paste `settings.json` fragment; output order is stable.
- [ ] Confirm failure, implement (reuse `RECURRENCE_THRESHOLD` / `CONFIRMED_DRIFT`), run tests (PASS).
- [ ] Commit `feat: propose hooks from recurring failure modes`.
...
- [ ] Confirm failure, implement the projection + seam, export from `improve/__init__.py`, run tests (PASS).
- [ ] Commit `feat: assemble improvement assets behind a narrator seam`.
...
- [ ] Confirm failure, implement the writer returning the four `Artifact`s, run tests (PASS).
- [ ] Commit `feat: write suggested instruction, hook, and suggestions artifacts`.
...
- [ ] Confirm failure, implement `run_suggest` (compose via `WorkflowRunner`, reuse `EvalHistoryReader` / `build_repo_memory` / `TraceWriter`) and the thin CLI adapter, run tests (PASS).
- [ ] Commit `feat: expose the agentops suggest command`.

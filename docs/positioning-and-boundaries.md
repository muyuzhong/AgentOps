...
护城河不在任何一个阶段，而在”仓库越来越适合 AI coding”这个累积效应本身。每一层单独看都可以被复制，但累积起来很难被替代：

| 阶段 | 护城河类型 | 强度 |
| --- | --- | --- |
| Phase 0–3 | 协议格式：`agentops-session.md` 的 task report 模板成为事实标准 | 薄 |
| Phase 3.5–4 | 对账方法论：声明 vs 真相的诊断规则，基于真实会话数据调校 | 中 |
| Phase 5–6 | 仓库记忆 + 可执行输出：累积的仓库经验 + 最后一公里的改进建议 | 厚 |
| Phase 7 | 集成深度：hook、Watcher、CLAUDE.md 持续维护——替换成本高 | 最厚 |

后来者需要同时复制：协议格式、诊断规则、仓库记忆、集成深度。这比复制任何一个单点功能都要困难得多。
...
3. Recommendation Engine
   ...
   - `CLAUDE.md` 优化不只是"加内容",也包括"做减法"。`CLAUDE.md` 每轮对话都会注入上下文,内容过多会挤占有效 token。优化目标是保持在 200 行以内,只保留项目特定的约束、验证命令和边界,移除教程、冗余说明和应该放在 README 里的内容。
   - 建议引擎不直接生成最终的 `CLAUDE.md` 文本,而是输出诊断结果（缺什么、多什么、哪些该精简）,并通过 skill 形式的优化指南让 coding agent 按规范执行实际改写。优化指南通过渐进式披露按需加载,避免污染 agent 的日常上下文。
...
- 异步检查点模式：coding agent 每完成一个独立任务后，按协议向 `.agentops/agentops-session.md` 追加结构化声明；AgentOps 拿这份声明去和 git diff、命令退出码、测试结果等 agent 无法伪造的 ground truth 对账。这种"声明 vs 真相"的差值诊断，比实时监听对话流更轻量、更可靠。
...
3. AgentOps Watcher
   ...
   - 核心机制不是"监听 Agent 对话",而是"对账 Agent 声明与实际证据"。Agent 每完成一个任务后产出结构化声明(做了什么、改了什么、验证了什么)；AgentOps 拿这份声明去和 git diff、命令退出码、测试结果等 Agent 无法伪造的 ground truth 对账。声明与真相之间的差值,才是诊断的核心信号。

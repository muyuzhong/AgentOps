...
- 当前分支：`main`
- 当前阶段：Phase 1 进行中，Task 1 到 Task 3 已完成。
- 当前版本：`0.1.0`
- 当前可用命令：
  - `agentops --help`
  - `agentops --version`
...
- 最近一次确认的测试结果：2026-05-30 执行 `python -m pytest -v`，共 `28 passed`。
...
| 路径 | 职责 |
| --- | --- |
| `agentops/cli.py` | 最小 CLI 入口。Phase 1 将新增 `scan` 子命令 |
...
```text
Task 4：实现 ReportWriter，写出 Markdown 和 JSON readiness 产物。
```

已完成：

- `RepoProfile.test_commands`
- `RepoScanner`
- `ReadinessEvaluator`
...
- `agentops scan` 尚未实现。
- 当前没有 session parser、runtime、writer、memory store 或 watcher。
